  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (isWebGL2) {
        ...
        // @ts-expect-error WebGL1 path
        halfFloat = gl.getExtension("OES_texture_half_float")
        supportLinearFiltering = !!gl.getExtension("OES_texture_half_float_linear")
      }
      gl.clearColor(0, 0, 0, 1)
      const halfFloatTexType = isWebGL2
        ? // @ts-expect-error webgl2 const
    ...
    function supportRenderTextureFormat(
      ...
      // @ts-expect-error overload difference
    ...
    function createProgram(vertexShader: WebGLShader | null, fragmentShader: WebGLShader | null): WebGLProgram | null {
      ...
      // Ensure attribute 0 is bound to aPosition to match our buffer/attrib pointer
    ...
    function scaleByPixelRatio(input: number) {
      const pixelRatio = Math.min(window.devicePixelRatio || 1, 2) // cap DPR for perf
    ...
    // Start loop (lazy-start on first move also works)
    rafId = requestAnimationFrame(updateFrame)
    ...
    return () => {
      ...
      // Best-effort cleanup of GL resources
      try {
        gl.bindFramebuffer(gl.FRAMEBUFFER, null)
      } catch {}
    }
